...
const map = {
  ...
  render(snakePointsArray, foodPoint, boardPoint) {
    ...
    const boardCell = this.cells[`x${boardPoint.x}_y${boardPoint.y}`];

    boardCell.classList.add('board');
    this.usedCells.push(boardCell);
  },
};
...
const game = {
  ...
  reset() {
    ...
    this.board.setCoordinates(this.getRandomFreeCoordinates());
  ...
  getRandomFreeCoordinates() {
    const exclude = [this.food.getCoordinates(), this.board.getCoordinates(), ...this.snake.getBody()];
    // without ... -  [{}, [{}, {}, {}]] => with ... [{}, {}, {}, {}];
    while (true) {
      const rndPoint = {
  ...
  render() {
    this.map.render(this.snake.getBody(), this.food.getCoordinates(), this.board.getCoordinates());
  },
  ...
  printCount() {
    this.point = document.querySelector(`.count`);
    this.point.innerHTML = '';
    this.point.insertAdjacentHTML('beforeend', 'ваш счет ' + this.quantity);
  },
  count() {
    this.quantity++
    console.log(this.quantity);
    const point = this.quantity
  },
  ...
  tickHandler() {
    ...
    if (this.board.isOnPoint(this.snake.getNextStepHeadPoint())) this.finish();
